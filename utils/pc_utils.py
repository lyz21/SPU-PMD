...
def xyzToPatch(file_path, target_folder, patch_num=1024, patch_num_ratio=4, DEVICE='cpu'):
    ...
    for point_path in test_files:
        ...
        save_folder = os.path.join(target_folder, point_path.split('/')[-1][
                                                  :-4])
        ...
        num_patches = int(data.shape[2] / patch_num * patch_num_ratio)
        print('num_patches:', num_patches)
        idx, seeds = operations.fps_subsample(data, num_patches, NCHW=True)
        patches, _, _ = operations.group_knn(patch_num, seeds, data,
                                             NCHW=True)
        print('patches.shape:', patches.shape)  # [B,C,group,N]，例：[1,3,320,256]
        for patch in patches:
            print('patch.shape:', patch.shape)  # [C,group,N]，例：[3,320,256]
            patch = patch.transpose(1, 0)
            print('patch.shape:', patch.shape)
            for i, p in enumerate(patch):
                p = p.transpose(1, 0)
                print('i:', i)
                np.savetxt(out_path[:-4] + '_' + str(i) + '.xyz', p.cpu(), fmt='%.6f')
