...
class ChamferLoss(torch.nn.Module):
    ...
    def forward(self, pred, gt, radius, p_norm=2):
        assert (pred.dim() == 3 and gt.dim() == 3), \
            "input for ChamferLoss must be a 3D-tensor, but pred.size() is {} gt.size() is {}".format(pred.size(),
                                                                                                      gt.size())
...
class OffsetLoss(torch.nn.Module):
    """
    INPUT： offset, TENSOR, [B,3,N]。example: [16,3,8192]
    ...
    def forward(self, offsets):
        ...
        for offset in offsets:
            loss = loss + torch.mean(offset ** 2)
        loss = loss / len(offsets)

        return loss


# UniformLoss --lyz 23.07.06
class UniformLoss(torch.nn.Module):
    """
    INPUT： pc,TENSOR, UniformLoss。example: [B,512,3]
    RETURN:tensor  Scalar
    """

    def __init__(self, loss_name='uniform', alpha=1):
        super(UniformLoss, self).__init__()
        from uniformLoss.loss import Loss as ULoss  # uniform loss，repulsion loss and so on
    ...
    def forward(self, pc):
        if self.name == 'uniform':
            loss = self.Loss.get_uniform_loss(pc) * self.alpha
        else:
            loss = self.Loss.get_repulsion_loss(pc) * self.alpha
