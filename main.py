...
from network.SPUPMD import SPUPMDNet as CRNet
...
parser.add_argument('--h5_data',
                    default='/home/tsmc/teamip/lyz/data/PU1K/train/pu1k_poisson_256_poisson_1024_pc_2500_patch50_addpugan.h5',
                    help='h5 file for training')

parser.add_argument('--test_data', default='/home/tsmc/teamip/lyz/data/PU1K/test/input_2048/input_2048',
                    help='test data path')
parser.add_argument('--gt_path', default='/home/tsmc/teamip/lyz/data/PU1K/test/input_2048/gt_8192',
...
def train(conf):
    ...
    net = net.cuda()
    net.train()
    model = Model(net, "train", conf, writer_tensorboard=writer_tensorboard)
...
def pc_prediction(net, input_pc, patch_num_ratio=3):
    ...
    for k in range(num_patches):
        ...
        patch, centroid, furthest_distance = operations.normalize_point_batch(
            patch, NCHW=True)  # example [1, 3, 24, 256]
        start = time.time()
...
def evaluation(target_folder, gt_folder, save_path):
    ...
    with open(os.path.join(save_path, "evaluation.csv"), "w") as f:
        ...
        # row["uniform (1e-3)"] = avg_uniform_value/counter *1000
        # print('uniform loss (p=0.004):', avg_uniform_value[0] / counter)
        # print('uniform loss (p=0.006):', avg_uniform_value[1] / counter)
        # print('uniform loss (p=0.008):', avg_uniform_value[2] / counter)
        # print('uniform loss (p=0.010):', avg_uniform_value[3] / counter)
        # print('uniform loss (p=0.012):', avg_uniform_value[4] / counter)
...
def online_evaluation(PRED_DIR, GT_DIR, save_path, writer_tensorboard=None, step=None):
    ...
    with open(os.path.join(save_path, "evaluation.csv"), "w") as f:
        ...
        row["CD (1e-3)"] = avg_cd_value * 1000.
        row["hausdorff (1e-3)"] = avg_hd_value * 1000.
        writer_tensorboard.add_scalar(tag="test/cd_loss", scalar_value=avg_cd_value, global_step=step)
        writer_tensorboard.add_scalar(tag="test/hd_loss", scalar_value=avg_hd_value, global_step=step)
        writer_tensorboard.add_scalar(tag="test/uniform_loss", scalar_value=avg_uniform_value, global_step=step)
